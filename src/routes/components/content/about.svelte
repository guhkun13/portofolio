<section id="whoami" class="pt-36 pb-32">
  <div class="container">
    <div class="flex flex-wrap lg:bg-transparent lg:backdrop-blur-md"> 
      <div class="w-full px-4 mb-10 lg:w-1/2">
        <div class="bg-black px-5 pt-3 pb-1 mb-5"> 
          <span class="font-bold font-mono text-green-500 text-xl mb-3"> $ whoami             
          </span>  
        </div>        
        <ul class="list-disc ml-5 font-mono space-y-3">          
          <li> Software Engineer | Fullstack Web </li>
          <li> Specialized as Backend Eng </li>          
          <li> Speak in Python, PHP, JavaScript, Go</li>          
          <li> Bachelor Degree of CS </li>
          <li> Eager to learn new things and innovate </li>
        </ul>        
        <p class="font-mono mt-5">
          Life is nor short and long. Live the life.
        </p>
        <div id="content" class="flex items-center mt-10">
          
          <!-- LinkedIn -->
          <a href="https://www.linkedin.com/in/guhkun13/" target="_blank" class="w-9 h-9 mr-3 rounded-full flex justify-center items-center border border-slate-300 hover:border-primary hover:bg-primary hover:text-blue-600 text-secondary">
            
            <svg width="20" class="fill-current" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>LinkedIn</title><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          </a>
          
          <!-- Twitter -->
          <a href="https://twitter.com/guhkun13" target="_blank" class="w-9 h-9 mr-3 rounded-full flex justify-center items-center border border-slate-300 hover:border-primary hover:bg-primary hover:text-blue-500 text-secondary">
            
            <svg width="20" class="fill-current" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Twitter</title><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>
          </a>
          
          <!-- Github -->
          <a href="https://github.com/guhkun13" target="_blank" class="w-9 h-9 mr-3 rounded-full flex justify-center items-center border border-slate-300 hover:border-primary hover:bg-primary hover:text-orange-500 text-secondary">
            
            <svg width="20" class="fill-current" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>GitHub</title><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>
          </a>
          
          <!-- Gitlab -->
          <a href="https://gitlab.com/guhkun13" target="_blank" class="w-9 h-9 mr-3 rounded-full flex justify-center items-center border border-slate-300 hover:border-primary hover:bg-primary hover:text-orange-500 text-secondary">
            
            <svg width="20" class="fill-current" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>GitLab</title><path d="m23.6004 9.5927-.0337-.0862L20.3.9814a.851.851 0 0 0-.3362-.405.8748.8748 0 0 0-.9997.0539.8748.8748 0 0 0-.29.4399l-2.2055 6.748H7.5375l-2.2057-6.748a.8573.8573 0 0 0-.29-.4412.8748.8748 0 0 0-.9997-.0537.8585.8585 0 0 0-.3362.4049L.4332 9.5015l-.0325.0862a6.0657 6.0657 0 0 0 2.0119 7.0105l.0113.0087.03.0213 4.976 3.7264 2.462 1.8633 1.4995 1.1321a1.0085 1.0085 0 0 0 1.2197 0l1.4995-1.1321 2.4619-1.8633 5.006-3.7489.0125-.01a6.0682 6.0682 0 0 0 2.0094-7.003z"/></svg>
          </a>
          
          <!-- Facebook -->
          <a href="https://web.facebook.com/guh.kun" target="_blank" class="w-9 h-9 mr-3 rounded-full flex justify-center items-center border border-slate-300 hover:border-primary hover:bg-primary hover:text-blue-700 text-secondary">
            
            <svg width="20" class="fill-current" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Facebook</title><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
          </a>
        </div>  
        
      </div>
      <div class="w-full px-4 mb-10 lg:w-1/2">
        <div class="mb-5">
          <div class="bg-black px-5 pt-3 pb-1 mb-5"> 
            <span class="font-bold font-mono text-green-500 text-xl mb-3"> $ tech-stack
            </span>  
          </div>
          <ul class="list-decimal ml-8 font-mono space-y-3">
            <li> 
              <div class="inline-flex items-center">
                <span class="w-9 h-9 mr-3 rounded-full flex justify-center items-center border border-slate-300 hover:border-primary hover:bg-green-500 hover:text-white text-secondary"> <svg role="img" width="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Python</title><path d="M14.25.18l.9.2.73.26.59.3.45.32.34.34.25.34.16.33.1.3.04.26.02.2-.01.13V8.5l-.05.63-.13.55-.21.46-.26.38-.3.31-.33.25-.35.19-.35.14-.33.1-.3.07-.26.04-.21.02H8.77l-.69.05-.59.14-.5.22-.41.27-.33.32-.27.35-.2.36-.15.37-.1.35-.07.32-.04.27-.02.21v3.06H3.17l-.21-.03-.28-.07-.32-.12-.35-.18-.36-.26-.36-.36-.35-.46-.32-.59-.28-.73-.21-.88-.14-1.05-.05-1.23.06-1.22.16-1.04.24-.87.32-.71.36-.57.4-.44.42-.33.42-.24.4-.16.36-.1.32-.05.24-.01h.16l.06.01h8.16v-.83H6.18l-.01-2.75-.02-.37.05-.34.11-.31.17-.28.25-.26.31-.23.38-.2.44-.18.51-.15.58-.12.64-.1.71-.06.77-.04.84-.02 1.27.05zm-6.3 1.98l-.23.33-.08.41.08.41.23.34.33.22.41.09.41-.09.33-.22.23-.34.08-.41-.08-.41-.23-.33-.33-.22-.41-.09-.41.09zm13.09 3.95l.28.06.32.12.35.18.36.27.36.35.35.47.32.59.28.73.21.88.14 1.04.05 1.23-.06 1.23-.16 1.04-.24.86-.32.71-.36.57-.4.45-.42.33-.42.24-.4.16-.36.09-.32.05-.24.02-.16-.01h-8.22v.82h5.84l.01 2.76.02.36-.05.34-.11.31-.17.29-.25.25-.31.24-.38.2-.44.17-.51.15-.58.13-.64.09-.71.07-.77.04-.84.01-1.27-.04-1.07-.14-.9-.2-.73-.25-.59-.3-.45-.33-.34-.34-.25-.34-.16-.33-.1-.3-.04-.25-.02-.2.01-.13v-5.34l.05-.64.13-.54.21-.46.26-.38.3-.32.33-.24.35-.2.35-.14.33-.1.3-.06.26-.04.21-.02.13-.01h5.84l.69-.05.59-.14.5-.21.41-.28.33-.32.27-.35.2-.36.15-.36.1-.35.07-.32.04-.28.02-.21V6.07h2.09l.14.01zm-6.47 14.25l-.23.33-.08.41.08.41.23.33.33.23.41.08.41-.08.33-.23.23-.33.08-.41-.08-.41-.23-.33-.33-.23-.41-.08-.41.08z"/></svg> </span> Python 🥇
              </div>
              <div class="font-mono mt-3 text-sm">
                <ul class="list-disc">
                  <li> Experience: 2017 - now  </li>
                  <li> Framework: Django, DRF, Flask </li>
                </ul>
              </div>
            </li>
            <li> PHP  🥈
              <div class="font-mono mt-3 text-sm">
                <ul class="list-disc">
                  <li> Experience: 2017  - now </li>
                  <li> Framework: Laravel, Yii1/2 </li>
                </ul>
              </div>
            </li>
            
            <li> 
              Javascript / nodejs 🥈
              <div class="font-mono mt-3 text-sm">
                <ul class="list-disc">
                  <li> Experience: 2021 - 2022 </li>
                  <li> Framework: NestJS </li>
                </ul>
              </div>
            </li>
            
            <li> GO 🥉
              <ul class="list-disc text-sm">
                <li> Experience: 2022 - now </li>
              </ul>
            </li>                       
          </ul>
        </div>
      </div>
      
      <div class="w-full px-4 mb-10 lg:w-1/2">
        <div class="bg-black px-5 pt-3 pb-1 mb-5"> 
          <span class="font-bold font-mono text-green-500 text-xl mb-3"> $ other tech skills 
          </span>  
        </div>        
        
        <div class="mb-2">
          <ul class="list-disc ml-5 font-mono space-y-3">            
            <li> GIT | <span class="text-xs"> using GITLAB and GITHUB ⛲ </span> </li>
            <li> Databases | <span class="text-xs"> MySql, Postgresql ⛲ </span> </li>
            <li> Linux |<span class="text-xs"> I love Open Source 💓 </span> </li> 
            <li> Docker | <span class="text-xs"> Contain(er) anything </span> 🥈 </li>
            <li> Networking |<span class="text-xs"> Basic </span> </li>
            <li> Bash Scripting | <span class="text-xs"> Basic </span></li>
            <li> HTML | <span class="text-xs"> Master </span> </li>
            <li> CSS | <span class="text-xs"> Bootstrap and tailwind CSS 🤟 </span> </li>
            <li> JavaScript | <span class="text-xs"> jQuery, and so on </span> </li>
          </ul>
        </div>                
      </div>
      
      <div class="w-full px-4 mb-10 lg:w-1/2">
        <div class="bg-black px-5 pt-3 pb-1 mb-5"> 
          <span class="font-bold font-mono text-green-500 text-xl mb-3"> $ others
          </span>  
        </div>        
        
        <div class="mb-2">
          <ul class="list-disc ml-5 font-mono space-y-3">
            <li> Bahasa Indonesia Profiency | 
              <span class="text-xs"> Native speaker of Bahasa Indonesia 🏆 </span> </li>
            <li> English Profiency | 
              <span class="text-xs"> Conversation, reading and writing 👍 </span> </li>
              <li> Problem Solving | 
                <span class="text-xs"> Understand the problem is the first step to solve it 😺 </span> </li> 
                <li> Communication |
                  <span class="text-xs"> The key is to understand each other 🔑 </span> </li>
                  <li> Agile (Scrum, Kanban) | 
                    <span class="text-xs"> Agile is flexible by stick to the rules </span> </li>
                    <li> Time Management | 
                      <span class="text-xs"> Health is top priority💗 </span></li>
                    </ul>
                  </div>                
                </div>
              </div>
            </div>
          </section>